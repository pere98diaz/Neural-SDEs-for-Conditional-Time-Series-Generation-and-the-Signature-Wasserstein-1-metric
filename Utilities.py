...
def plot_baseline(G, X_data, Y_data, nsamples_fs, device='cuda'):
    ...
    with torch.no_grad():
        ...
        pred_Y = G(x=X_mc[:, :, 1:], steps=q).view(nsamples_fs, nsamples, q+1, 1)
...
def plot_nsde(G, X_data, Y_data, sig_X, nsamples_fs, device='cuda'):
    ...
    with torch.no_grad():
        ...
        pred_Y = G(sig_X_mc, X_mc[:, :, 1:], q).view(nsamples_fs, nsamples, q+1, 1)
    ...
    pos = np.array([[0,0], [0,1], [1,0], [1,1]])
    for i in range(4): 
        axs[pos[i, 0], pos[i, 1]].plot(X[i, :, 0], X[i, :, 1], color='dodgerblue', linewidth=0.7, alpha=1)
        axs[pos[i, 0], pos[i, 1]].plot(Y[i, :, 0]+X[i, -1, 0], Y[i, :, 1], color='green', linewidth=0.7, alpha=1)
        for j in range(nsamples_fs):
            axs[pos[i, 0], pos[i, 1]].plot(Y[i, :, 0]+X[i, -1, 0], pred_Y[j, i, :, 0], color='crimson', linewidth=0.7, alpha=0.9)    
    plt.show()
